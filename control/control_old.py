...
class Executable:
    ...
    def parse_dict(self, data):
        ...
        if not os.access(resolve(self.args[0]), os.X_OK) and self.args[0].endswith(
            ".js"
        ):
...
class Config:
    ...
    def parse_dict(self, data):
        ...
        for key, value in data.pop("services", {}).items():
            ...
            if len(tmp.keys()):
                print(
                    "WARNING: service %s has additional keys %r"
                    % (key, list(tmp.keys()))
                )
        ...
        if len(data.keys()):
            print(
                "WARNING: configuration has additional keys %r" % list(data.keys()),
            )
...
class SystemD:
    ...
    def service_template(self, service):
        ...
        tpl += "ExecStart=%s\n" % (" ".join([shlex.quote(i) for i in service.args]),)
        ...
        try:
            self.run(
                [
                    "systemctl",
                    "start",
                    service.config.name + "-" + service.name + ".service",
                ]
            )
            time.sleep(1)
            self.run(
    ...
    def stop(self, service):
        ...
        self.run(
            ["systemctl", "stop", service.config.name + "-" + service.name + ".service"]
        )
        ...
        if service.type == "daemon":
            ...
            self.run(
                [
                    "systemctl",
                    "enable",
                    service.config.name + "-" + service.name + ".timer",
                ]
            )
            self.run(
                [
...
def main():
    ...
    if True:
        ...
        parser.set_defaults(
            func=lambda args: commands.status(names=args.name, full=args.full)
        )
    ...
    if True:
        ...
        parser.set_defaults(
            func=lambda args: commands.log(names=args.name, follow=args.follow)
        )
