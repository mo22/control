...
## Live macOS services: ExitTimeOut=30 rolled out (DONE 2026-09-24, Moritz approved)

All **18** control-managed launchd services (they had `ExitTimeOut` unset) were
reinstalled with the repo's new code (`uv run control … install`) and now carry
`ExitTimeOut=30`. Verified: **18/18 ExitTimeOut=30, 18/18 loaded**, all
WorkingDirectories/executables resolve to their correct locations, all pre-restart
pids dead, and **no PPID-1 orphans and no duplicate instances** after the restarts.
The orphan-prone tcc-venv/uvx services (`fileindex watch`, `mac-mcp`, `webshell`,
`webshell-agent-mcp`, `whatsapp-watcher`, `winmgr`) were brought down with the smart
`control stop` first (which cleanly collapsed the whole tree — confirming the
running fileindex payload already had the SIGTERM fix) and then reinstalled.

Services (grouped by `control.yaml`): `claude-remote-mcp` (chrome-mcp,
gateway-agent/chrome/mac, mac-mcp, tunnel-agent/chrome/mac, webshell-agent-mcp);
`columbo`; `file-butler` (kicktipp-watcher, whatsapp-watcher); `fileindex` (watch ←
the incident service); `hav-assistant` (gpt, respeaker); `webshell`
(tunnel-devmxsde, webshell); `winmgr`.

`control.log-rotation` is a separate codepath (`install-log-rotation`, not
`_generate_plist`) and does not get `ExitTimeOut` — a periodic sweep, not a daemon,
so it does not matter.

### Gotcha found during rollout: relative `cwd` resolves against the invoking shell

The `webshell.webshell` service uses `run: ./webshell-tcc` + `cwd: backend`
(relative). `control`'s `to_executable_args` resolves a relative `cwd:` with
`os.path.realpath(self.cwd)` — i.e. **against the process's current directory, not
the config file's directory** (`control/models.py:57`). Running
`control … install webshell` from the control repo therefore failed with
`Executable does not exist: …/control/backend/webshell-tcc`, and `install()`
unloads the old job *before* `_generate_plist` raises, so the service was briefly
left down. Recovered by reloading the intact old plist, then reinstalling from
`~/workspace/webshell` (`uv run --project ~/workspace/control control …`). **Any
future `control install` for a service with a relative `cwd:` must run from that
service's expected working directory.** Pre-existing behavior; left as-is (not part
of this task) but worth flagging — arguably a control bug (relative `cwd:` should
resolve against the config dir like the default does).

### Note on the installed CLI still being 0.10

These plists were written by the repo's code via `uv run control`; the per-user
installed `control` CLI is still 0.10 (no `ExitTimeOut`). A future plain
`control install` from the old tool would overwrite `ExitTimeOut` back to unset.
The durable fix is a release (bump version, tag, `uv tool install --force`) — a
separate step that needs Moritz's go for tagging.
